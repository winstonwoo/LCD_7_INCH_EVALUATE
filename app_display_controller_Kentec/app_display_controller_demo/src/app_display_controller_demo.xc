...
#elif defined DEF_KETECK
on tile[0] : lcd_ports lcdports = { XS1_PORT_1E, XS1_PORT_1F, XS1_PORT_32A, XS1_CLKBLK_1 };
on tile[0] : sdram_ports sdramports = {XS1_PORT_16A, XS1_PORT_4F, XS1_PORT_1H, XS1_CLKBLK_2 };
on tile[0] : touch_controller_ports touchports = {
        {XS1_PORT_1I, XS1_PORT_1J, 1000}, XS1_PORT_1K };
on tile[0] :out port  bo = XS1_PORT_1G;
#else  //DEF_SKW
on tile[0] : lcd_ports lcdports = { XS1_PORT_1E, XS1_PORT_1F, XS1_PORT_32A, XS1_CLKBLK_1 };
on tile[0] : sdram_ports sdramports = {XS1_PORT_16A, XS1_PORT_4F, XS1_PORT_1H, XS1_CLKBLK_2 };
on tile[0] : touch_controller_ports touchports = {
        {XS1_PORT_1L, XS1_PORT_1M, 1000}, XS1_PORT_1K };
on tile[0] :out port  bo = XS1_PORT_1G;
#endif
/*
...
static void load_image(chanend c_c_server, chanend c_loader, unsigned image_no) {
  unsigned buffer[480];
  for (unsigned line = 0; line < 272; line++){
...
void app(chanend c_server, chanend c_loader){
  ...
  bo <: 1 ;
  for(unsigned i=0;i<IMAGE_COUNT;i++){
    image[i] = display_controller_register_image(c_server, 240, 272);
    load_image(c_server, c_loader, image[i]);
  }

  frame_buffer[0] = display_controller_register_image(c_server, 240, 272);
  frame_buffer[1] = display_controller_register_image(c_server, 240, 272);
  display_controller_frame_buffer_init(c_server, image[0]);
  touch_lib_init(touchports);
  printstrln("****** Please touch any of the corners or the center of LCD screen ******");
  printstrln("******              to see different transition effects            ******");
  ...
  while(1){
    ...
    if (x<480/3 && y<272/3){
        fb_index = transition_slide(c_server, frame_buffer, image[current_image],
                image[next_image],240, fb_index);
    } else if (x>2*480/3 && y<272/3){
        fb_index = transition_dither(c_server, frame_buffer, image[current_image],
                image[next_image], 256, fb_index);
    } else if (x>2*480/3 && y>2*272/3){
        fb_index = transition_wipe(c_server, frame_buffer, image[current_image],
                image[next_image], 240, fb_index);
    } else if (x<480/3 && y>2*272/3){
        fb_index = transition_roll(c_server, frame_buffer, image[current_image],
                image[next_image], 240, fb_index);
    } else if (x>480/3 && x<2*480/3 && y>272/3 && y<2*272/3){
        fb_index = transition_alpha_blend(c_server, frame_buffer, image[current_image],
                image[next_image], 64, fb_index);
    }
    current_image = next_image;
...
int main() {
  ...
  par {
	  ...
	  on tile[0]:loader(c_loader, images, IMAGE_COUNT);
  }
  return 0;
}
